...
$( document ).ready(function()
{
                ...
                success: function ( password )
                {
                    ...
                    call_api_ajax('ReadUserJSONSettings', 'get', { SessionToken: data.Result.SessionToken }, false, ( data ) =>
                    {
                        ...
                        if ( data !== "" )
                        {
                            ...
                            setCookie('remember-id',        parseInt(data.slice(0, 1)));
                            setCookie('remember-username',  parseInt(data.slice(1, 2)));
                            setCookie("remember-password",  parseInt(data.slice(2, 3)));
                            setCookie("remember-checkbox",  parseInt(data.slice(3)));

                            if (parseInt(data.slice(0, 1))) setCookie('id', dataP.UserReferenceNo);
                            if (parseInt(data.slice(1, 2))) setCookie('username', dataP.Username);
                            if (parseInt(data.slice(2, 3))) setSession( password, "password" );
                        }
                    });
    ...
    $("#loginPopup").css("min-width", 320).css("min-height", 409);
            ...
            if(getSession() == undefined || getSession() == ""){
                ...
                (data) => {
                    ...
                    function CreateAddHeaderRow() {
                        ...
                        //     $("#reportPadlock").prop('checked', false);
                        //     $("#reportName").val('');
                        //     $('#addReportWindow').dialog('open');
                        //     $("#reportName").focus();
                        // });
                    ...
                    $('#reportBackups').on('resized', function (event) {
                        ...
                        report_columns[1].width = parseInt($("#reportBackups").width()) - 485;
